...
function getEvents(recording) {
    return recording?.events ?? [];
}
function isRecord(value) {
    return typeof value === 'object' && value !== null;
}
function getToolCallsFromPayload(payload) {
    if (!isRecord(payload) || !Array.isArray(payload['tool_calls'])) {
        return [];
    }
    return payload['tool_calls'];
}
function getToolResultsFromPayload(payload) {
...
function getStopReasonFromPayload(payload) {
    if (!isRecord(payload)) {
        return null;
    }
...
function getTotalTokensFromPayload(payload) {
    if (!isRecord(payload)) {
        return null;
    }
    return typeof payload['total_tokens'] === 'number' ? payload['total_tokens'] : null;
}
function getConstraint(session) {
    return session?.constraint;
}
function getUserInteractionCount(input) {
    const messageCount = input.messages?.filter((message) => message.role === 'user').length ?? 0;
    if (messageCount > 0) {
        return messageCount;
    }
    const eventCount = getEvents(input.recording).filter((event) => event.type === 'message' && getRoleFromPayload(event.payload) === 'user').length;
    if (eventCount > 0) {
        return eventCount;
    }
    return input.session?.interactions_used ?? 0;
}
function buildInteractionWindows(events) {
    const windows = [];
    let current = null;
...
function countProductiveInteractions(input) {
    ...
    if (windows.length > 0) {
        return windows.filter((window) => hasMeaningfulAgentResponse(window.events) || hasSuccessfulToolResult(window.events)).length;
    }
    const assistantMessages = input.messages?.filter((message) => message.role === 'assistant' && (message.content ?? '').trim().length > 0).length ?? 0;
    const totalInteractions = getUserInteractionCount(input);
    return Math.min(totalInteractions, assistantMessages);
}
function getTotalTokensUsed(input) {
...
function getCorrectnessScore(input) {
...
function getManualEditedLineCount(recording) {
...
function getFinalLineCount(input) {
    if (input.finalFiles) {
...
function getToolResultEntries(recording) {
    const entries = [];
...
function getAgentResponseEntries(recording) {
    const entries = [];
    for (const [index, event] of getEvents(recording).entries()) {
        if (event.type !== 'agent_response') {
            continue;
...
export function computeIterationEfficiency(input) {
    const totalInteractions = getUserInteractionCount(input);
...
export function computeTokenEfficiency(input) {
...
export function computeIndependenceRatio(input) {
...
export function computeRecoveryScore(input) {
    ...
    const errorEntries = [
        ...toolResults.filter((entry) => entry.isError).map((entry) => ({
            index: entry.index,
            matchesRecovery: (candidate) => !candidate.isError && candidate.name === entry.name,
        })),
        ...agentResponses.filter((entry) => entry.isError).map((entry) => ({
            index: entry.index,
            matchesRecovery: (_candidate) => false,
        })),
    ];
    ...
    const corrected = errorEntries.filter((entry) => {
        const recoveredByTool = toolResults.some((candidate) => candidate.index > entry.index && entry.matchesRecovery(candidate));
        const recoveredByResponse = agentResponses.some((candidate) => candidate.index > entry.index && candidate.isRecovery);
